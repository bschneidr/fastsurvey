...
$$\hat F(x) = \frac{\sum_i w_i}{\sum_i \{x_i\leq x\}w_i}$$
...
higher observation. This is the same as  \texttt{type=2} in \texttt{quantile}.
...
data values are sorted.   This is the same as  \texttt{type=3} in \texttt{quantile}.
...
\sout{a worthwhile}\dots\sout{an interesting}... a research question for
simulation.
\end{document}
